...
export default function AdminNoticePage() {
    ...
    function resetForm() { setForm(EMPTY); setEditId(null); setError(""); }
        ...
        try {
            if (form.is_active) {
                const { error: deactivateErr } = await supabase
                    .from("notices").update({ is_active: false }).neq("id", editId ?? -1);
        ...
        <div style={{ maxWidth: 1040, fontFamily: "'Inter',sans-serif" }}>
            ...
            <div style={{ marginBottom: 28 }}>
                ...
                <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 14 }}>
                    Manage the popup visitors see on the homepage. One notice active at a time.
                </p>
            </div>
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    boxShadow: activeNotice ? "0 0 8px #8DC63F" : "none", flexShrink: 0,
                }} />
                ...
                {activeNotice?.image_url && (
                    <img src={activeNotice.image_url} alt=""
                        style={{ width: 56, height: 36, objectFit: "cover", borderRadius: 6, flexShrink: 0, opacity: 0.85 }} />
                )}
            </div>
            ...
            <div style={{ ...S.card, marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18, flexWrap: "wrap", gap: 8 }}>
                    ...
                    {editId && <button style={S.btnSec} onClick={resetForm}>Cancel edit</button>}
                </div>
                ...
                <div style={{ marginBottom: 16 }}>
                    ...
                    <input style={S.input} placeholder="e.g. Diwali Sale — 20% Off All Products"
                        value={form.title} onChange={(e) => setF("title", e.target.value)} maxLength={80} />
                ...
                <div style={{ marginBottom: 16 }}>
                    ...
                    <textarea style={{ ...S.input, resize: "vertical", minHeight: 90 }}
                        placeholder="e.g. Shop closed 1–5 November for the festive season. Place orders early."
                        value={form.message} onChange={(e) => setF("message", e.target.value)} maxLength={300} />
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", marginTop: 4 }}>{form.message.length}/300</div>
                </div>

                {/* Image upload */}
                <div style={{ marginBottom: 16 }}>
                    <label style={S.label}>Notice image (optional)</label>
                    <input
                        ref={fileRef}
                        type="file"
                        accept="image/*"
                    ...
                    {form.image_url ? (
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            <img src={form.image_url} alt="Notice"
                                style={{ width: 120, height: 72, objectFit: "cover", borderRadius: 8, border: "1px solid rgba(255,255,255,0.1)" }} />
                            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                                <button type="button" style={S.btnSec} onClick={() => fileRef.current?.click()} disabled={imgUploading}>
                                    {imgUploading ? "Uploading…" : "Replace image"}
                                </button>
                                <button type="button" style={S.btnDanger} onClick={() => setF("image_url", "")}>
                                    Remove image
                                </button>
                            </div>
                        </div>
                    ) : (
                        <button type="button" style={{ ...S.btnSec, width: "100%", padding: "18px 12px", borderStyle: "dashed" }}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                    <div>
                        ...
                        <input style={S.input} placeholder="e.g. View Offers"
                            value={form.cta_label} onChange={(e) => setF("cta_label", e.target.value)} maxLength={30} />
                    </div>
                    <div>
                        <label style={S.label}>CTA link (optional)</label>
                        <input style={S.input} placeholder="e.g. /products or https://…"
                            value={form.cta_link} onChange={(e) => setF("cta_link", e.target.value)} />
                    </div>
                </div>
                        ...
                        {notices.map((n) => {
                            ...
                            return (
                                <div key={n.id} style={{
                                    ...
                                    {/* Type badge (image thumbnail commented out — IMAGE FEATURE) */}
                                    ...
                                    <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                                        <button
                                            style={{ padding: "6px 12px", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer", border: `1px solid ${n.is_active ? "rgba(239,68,68,0.3)" : "rgba(141,198,63,0.3)"}`, background: n.is_active ? "rgba(239,68,68,0.1)" : "rgba(141,198,63,0.1)", color: n.is_active ? "#f87171" : "#8DC63F" }}
                                        ...
                                        <button
                                            style={{ padding: "6px 12px", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer", border: confirmDel === n.id ? "1px solid #ef4444" : "1px solid rgba(239,68,68,0.25)", background: confirmDel === n.id ? "rgba(239,68,68,0.25)" : "rgba(239,68,68,0.08)", color: "#ef4444" }}
